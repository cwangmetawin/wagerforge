...
## Process
1. Classify every economy flow. Faucets (value entering player balances): wins, bonus credits, loyalty/cashback. Sink (value permanently exiting): the house edge realized on settled wagering — i.e. `sink = Σ wager × houseEdge`, not gross wager turnover.
...
5. Attribute moves: split faucet into wins vs bonus vs loyalty so you know which lever caused the imbalance before changing RTP, bonus generosity, or loyalty accrual.
...
- Ignoring loyalty/cashback as a faucet; it is real outflow that compounds.
