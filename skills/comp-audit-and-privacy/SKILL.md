...
3. Apply GDPR: pseudonymize player identifiers in logs, encrypt at rest and in transit, gate reads with RBAC, and enforce per-data-class retention/deletion timelines (fines up to 4% of annual turnover / €20M).
...
5. Dark-pattern audit: keep responsible-gaming controls (limits, self-exclusion, reality checks) at least as prominent and as easy to action as promotions; no pre-ticked opt-ins, hidden exits, or confirm-shaming.

## Pitfalls / red flags
- Treating audit logs as optional or mutable (UPDATE/DELETE on a settled record breaks immutability).
- Raw PII in logs instead of pseudonymized references; unencrypted at rest or in transit; no RBAC on log reads.
...
- Promotions louder than RG controls, pre-checked consents, friction asymmetry (easy to deposit, hard to set a limit) — all dark patterns.
